...
class NodeVisitor:
    ...
    def visit_reference(self, node):
        ...
        if refuri == "":
            ...
            # section headings tend to differ from the IDs generated by Docutils.
            # cf. https://docutils.sourceforge.io/docs/ref/doctree.html#identifiers
            if isinstance(target_node, nodes.section):
                title = target_node[0]
                if refid == nodes.make_id(title.astext()):
                    if 'auto' in title:
                        # automatic section numbering -> unpredictable MoinMoin-IDs
                        node.document.reporter.warning(
                            "ID of auto-numbered section headings irreproducible in rST."
                            " Use an explicit target or turn off section numbering.",
                            base_node=node)
                    else:
                        # replace reference ID made by Docutils with ID made by Moin:
                        refid = anchor_name_from_text(" ".join(title.astext().split()))
...
class Parser(docutils.parsers.rst.Parser):
    ...
    Derived from the Docutils rST parser, this class defines and registers
    a "resolver" method for hyperlink references without a matching target
    (cf. `docutils.TransformSpec.unknown_reference_resolvers`).
    """
    ...
    def __init__(self):
        super().__init__()
        self.unknown_reference_resolvers = [self.wiki_reference_resolver]
        self.wiki_reference_resolver.priority = 400

    @staticmethod
    def wiki_reference_resolver(node):
        """Resolve unknown references as local wiki references.

    Named hyperlink references
    point to a wiki page with the name derived from the
    reference text (replacing whitespace with "_").

        The node's text is converted to a URI reference with
...
class Writer(writers.Writer):
    # Note: In moin 2.0, the Writer class is no longer used!
