...
*
* Copyright (c) 2026 MoinMoin project
*
...
;( function( $ ) {
	...
	const policy = window?.trustedTypes?.createPolicy("tablesorter", {
		...
		*/
		...
		buildHeaders : function( c ) {
			...
			c.$headers = $( $.map( c.$table.find( c.selectorHeaders ), function( elem, index ) {
				...
				c.headerContent[ index ] = policy.createHTML($elem.html());
				...
				if ( c.headerTemplate !== '' && !$elem.find( '.' + ts.css.headerIn ).length ) {
					...
					$elem.html( policy.createHTML('<div class="' + ts.css.headerIn + '">' + template + '</div>') ); // faster than wrapInner
				}
				if ( c.onRenderHeader ) {
		...
		*/
		...
		fixColumnWidth : function( table ) {
			...
			if ( c.widthFixed && c.$table.children( 'colgroup' ).length === 0 ) {
				$colgroup = $( policy.createHTML('<colgroup class="' + ts.css.colgroup + '">') );
		...
		processTbody : function( table, $tb, getIt ) {
			...
			if ( getIt ) {
				table.isProcessing = true;
				$tb.before( policy.createHTML('<colgroup class="tablesorter-savemyplace"/>') );
